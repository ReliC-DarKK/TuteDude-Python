# TuteDude-Python
Assignments for the python course by Rudransh Kumar Singh

<h1><b>
  ASSIGNMENT 1
</b></h1>

<h2>
  Task 1: Perform Basic Mathematical Operations</h2>
  <h3> It is a basic calculator that performs all basic mathematical functions.
  <br>
  It inputs two float-type numericals from user using input function and performs addition, subractiion, multiplication and division. It then prints the values using print function.
  </h3>

<h2>
  Task 2: Create a Personalized Greeting </h2>
  <h3>
  It inputs the first name and last name from user and greets them using print function.
  </h3>

<br>
<br>
<br>

<h1><b>
  ASSIGNMENT 2
</b></h1>

<h2>
  Task 1: Check whether the number is odd or even.</h2>
  <h3> It uses the '%' function for remainder.
  <br>
  It inputs an integer from user. 
  If the remainder of the number on dividing by 2 is zero, the number is even.
  Else it is odd.
  </h3>

<h2>
  Task 2: Count from 1 to 50 </h2>
  <h3>
  The logic uses a for loop to iterate over a sequence of numbers. The range() function generates integers starting from the first value (1) and stopping before the second value (51), ensuring numbers up     to 50 are included. Each number i in this sequence is then printed. 
  </h3>
